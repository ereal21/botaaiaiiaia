...
def update_promocode(code: str, discount: int | None = None, expires_at: str | None = None) -> None:
    ...
    if discount is not None:
        update_data[PromoCode.discount] = discount
    if expires_at is not None:
        update_data[PromoCode.expires_at] = expires_at
    if not update_data:
        return
    Database().session.query(PromoCode).filter(PromoCode.code == code).update(values=update_data)
    Database().session.commit()
